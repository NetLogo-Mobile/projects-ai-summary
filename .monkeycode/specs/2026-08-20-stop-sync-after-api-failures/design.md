...
标签同步使用一个任务级 API 失败守卫跟踪连续失败次数。所有标签同步 API 请求共享该守卫，成功请求清零计数，第二次连续失败产生停止信号。分页任务顺序调度，使停止信号能够阻止后续分页开始执行。作品摘要进入标签计算前执行内容过滤，排除带有“小作品”标签和正文少于 50 个字符的作品。
...
守卫维护单个整数 `consecutiveFailures`。初始值为零，成功调用写入零，失败调用加一，达到二时抛出停止信号。
...
- 带有“小作品”标签的作品不会调用标签更新 API。
- 去除正文两端空白后合计少于 50 个字符的作品不会调用标签更新 API。
