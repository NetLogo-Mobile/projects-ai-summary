...
async function maybeExpandFilters(
  ...
  const originalKeywords =
    filters.keywords?.map((x) => x.trim()).filter(Boolean) ?? [];
  if (originalKeywords.length === 0) {
    return { expanded: filters, extraKeywords: [] };
  }

  const { extraKeywords } = await expandKeywordsWithGroq(
    content,
    originalKeywords,
  );
...
async function processQueryMessage(content: string): Promise<string> {
  ...
  const { expanded, extraKeywords } = await maybeExpandFilters(
    content,
    filters,
  );
  const rows = await searchRecords(expanded);
  if (rows.length === 0) {
    let re = "未命中记录，请缩小条件或更换关键词。\n" + helpMessage();
    if (filters.keywords && filters.keywords.length > 0) {
      re += `关键词: ${filters.keywords.join(", ")}\n`;
    }
    if (extraKeywords.length > 0) {
      re += `模糊扩展: ${extraKeywords.join(", ")}\n`;
    }
  }
  ...
  const lines = rows.map((x) => {
    const keywords = x.keyWords
      ? JSON.parse(x.keyWords).slice(0, 3).join(", ")
      : "";
...
export async function runDiscussionBot(): Promise<void> {
  const bot = new pl.Bot(
    ...
    (msg: any) => {
      console.log(
        `[Bot] 捕获消息: ${msg.ID} 来自 ${msg.Nickname}: ${msg.Content.substring(0, 50)}`,
      );
    },
    // replyed：成功回复后的回调
    (msg: any) => {
